...
import { Heart, Sparkles, Calendar, Clock, Image, Mic, Dices, ArrowRight, Quote, ChevronDown, Star, Wand2, X } from 'lucide-react';
...
export const LandingPage = ({ setActiveTab }) => {
  ...
  const surprises = [
    { kind: 'memory', title: 'A little memory', text: 'Go look at a moment you two decided was worth keeping.', tab: 'memories', icon: Image },
    { kind: 'activity', title: 'Date night idea', text: 'Let tonight be a little less planned and a little more yours.', tab: 'activities', icon: Dices },
    { kind: 'voice', title: 'Hear a familiar voice', text: 'There might be a little piece of you waiting in Voice of Us.', tab: 'voicenotes', icon: Mic },
    { kind: 'journal', title: 'Open the pages', text: 'Somewhere in your story, there is a sentence worth reading again.', tab: 'journal', icon: BookHeart },
  ];
  const openSurprise = () => setSurprise(surprises[Math.floor(Math.random() * surprises.length)]);
  ...
  return <div className="relative min-h-[calc(100vh-5rem)] overflow-hidden pb-24">
    {surprise && (() => { const Icon = surprise.icon; return <div className="fixed inset-0 z-[70] bg-velvet-950/80 backdrop-blur-xl flex items-center justify-center p-5" onClick={()=>setSurprise(null)}><div onClick={e=>e.stopPropagation()} className="w-full max-w-lg rounded-[2rem] border border-rose-300/20 bg-[#fff8ec] text-[#35151e] p-7 sm:p-10 shadow-2xl text-center"><button onClick={()=>setSurprise(null)} className="ml-auto w-10 h-10 rounded-full bg-[#5a1c2c]/10 text-[#5a1c2c] flex items-center justify-center"><X/></button><div className="mx-auto w-16 h-16 rounded-2xl bg-[#f4dbe2] flex items-center justify-center mt-2"><Icon className="w-7 h-7 text-[#7a243c]"/></div><div className="text-[10px] uppercase tracking-[.3em] text-[#8b5362] mt-5">Your surprise</div><h2 className="font-serif text-4xl italic mt-2">{surprise.title}</h2><p className="mt-4 text-[#6f4b55] leading-relaxed">{surprise.text}</p><button onClick={()=>{setSurprise(null);navigate(surprise.tab)}} className="mt-7 rounded-full bg-[#5a1c2c] px-6 py-3 text-sm font-semibold text-white">Open it ❤️</button></div></div>})()}
