...
const MainApp = () => {
  ...
  useEffect(() => { setHearts(Array.from({ length: 12 }).map((_, i) => ({ id: i, left: `${Math.random() * 100}%`, size: `${Math.random() * 14 + 10}px`, duration: `${Math.random() * 10 + 14}s`, delay: `${Math.random() * 8}s` }))); }, []);
